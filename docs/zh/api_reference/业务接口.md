...
- URL：`http(s)://{CoordinatorIP}:{推理端口}/v1/chat/completions`
  ...
  > **说明**
  >
  > `{CoordinatorIP}`：Coordinator 服务部署机器的 IP 或域名，取值来自配置 `http_config.coordinator_api_host`（默认 `127.0.0.1`），参考 `examples/infer_engines/vllm/user_config.json` 等（以实际使用的配置为准）取值或实际运行时节点 IP；`{推理端口}`：内部端口取值来自于配置项 `http_config.coordinator_api_infer_port`（默认 `1025`），对外绑定端口由部署配置中的 `spec.ports[].nodePort` 指定（默认 `31015`），具体可参考 `examples/deployer/yaml_template/coordinator_template.yaml`（multi_deployment 场景）或 `examples/deployer/yaml_template/infer_service_template.yaml`（CRD 场景）
- 请求头：
