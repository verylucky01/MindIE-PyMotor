...
- URL：`http(s)://{CoordinatorIP}:{管理端口}/startup`
  ...
  > **说明**
  >
  > `{CoordinatorIP}`：Coordinator 服务部署机器的 IP 或域名，取值来自配置 `http_config.coordinator_api_host`（默认 `127.0.0.1`），参考 `examples/infer_engines/vllm/user_config.json` 等（以实际使用的配置为准）取值或实际运行时节点 IP；`{管理端口}`：配置项 `http_config.coordinator_api_mgmt_port`（默认 `1026`）
