...
请将本仓库中的 **deployer** 目录上传至 K8s 集群的 master 节点。与 PD 分离部署相关的主要目录结构如下：

```text
deployer/
├── user_config.json          # 部署与业务配置入口（必改）
├── deploy.py                  # 部署入口脚本
├── delete.sh                  # 卸载脚本
├── show_log.sh                # 可选：启动日志采集/查看
├── conf/
│   └── env.json               # 环境变量配置（可选改）
├── boot_helper/
│   ├── boot.sh                # 容器内启动脚本（由 deploy.py 注入 env）
│   └── hccl_tools.py          # 生成 ranktable 等
├── deployment/                # K8s 模板
│   ├── controller_init.yaml
│   ├── coordinator_init.yaml
│   └── engine_init.yaml       # P/D 实例共用模板
├── probe/
│   ├── probe.sh               # 启动/就绪/存活探针
│   └── probe.py
├── log_collect/               # 可选：日志采集
...
│   └── log_monitor.py
└── output/
    └── deployment/            # deploy.py 生成的 YAML（供 delete.sh 卸载）
```

- `user_config.json`：部署与业务的唯一必改入口，详见下文。
- `env.json`：可按需修改各组件环境变量；不修改则使用默认或模板值。
- `deploy.py`：读取 `user_config.json` 及 `env_path` 指向的 `env.json`，生成 YAML 并执行部署。
...
- `--user_config_path`：默认 `./user_config.json`，可指定其它路径。
- `--update_config`：仅刷新 ConfigMap（motor-config），不重新 apply Deployment。

示例：

```bash
python3 deploy.py --user_config_path ./user_config.json
```

`deploy.py` 会依次执行以下步骤：

1. 读取 `user_config.json` 和 `env_path` 指向的 `env.json`。
2. 根据 `motor_deploy_config`，生成 Controller、Coordinator 及各 P/D 的 Deployment YAML 到 `output/deployment/`。
3. 将 `env.json` 中的环境变量写入 `boot_helper/boot.sh` 的对应函数。
...
在 **deployer** 目录下执行 `delete.sh`，将删除当前 `job_id` 对应命名空间下的 ConfigMap 以及 `output/deployment/` 中已 apply 的 YAML，并清理 `boot.sh` 中由 `deploy.py` 注入的环境变量函数。
...
为简化使用，若用户拉起服务的配置文件名未改动（user_config.json），或配置文件中使用的命名空间为默认的 `mindie-motor` 则可以不用传参：
...
>- `delete.sh` 会删除命名空间下的 motor-config ConfigMap 以及 `output/deployment` 下已 apply 的 YAML，并清理 `boot.sh` 中由 `deploy.py` 注入的各 `set_*_env` 函数。卸载脚本**必须在 deployer 目录下**执行，否则无法正确找到 output 路径而报错。
